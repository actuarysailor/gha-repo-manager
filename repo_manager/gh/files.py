...
def __clone_repo__(repo: Repository, branch: str) -> Repo:
    ...
    cloned_repo = Repo.clone_from(
        repo.clone_url.replace("https://", f"https://{inputs['username']}:{inputs['token']}@"), str(repo_dir)
    )
    ...
    for branch in branches:
        ...
        if branch.target_branch in set(diffs.keys()):
            ...
            if push_errors:
                ...
                existing_prs = list(repo.get_pulls(state="open", head=f"{repo.owner.login}:{target_branch}", base=branch.target_branch))
                if existing_prs:
                    pr = existing_prs[0]
                    pr.edit(body=body)
                    actions_toolkit.info(
                    ...
                    actions_toolkit.info(
                        f"Created pull request for branch {target_branch} → {branch.target_branch}"
                    )
