...
from repo_manager.schemas.environment import environment, Reviewer, DeploymentBranchPolicy
...
def __create_environment_branch_policy(repo: Repository, environment_name: str, branch_name_pattern: str) -> bool:
    ...
    status, headers, data = repo._requester.requestJson(
        "POST", f"{repo.url}/environments/{environment_name}/deployment-branch-policies", input=put_parameters
    )
    if status not in {200}:
        raise Exception(f"Unable to create deployment branch policy for environment {environment_name} with branch pattern {branch_name_pattern}. Status: {status}. Error: {json.loads(data)['message']}")
...
def __delete_environment_branch_policy(repo: Repository, environment_name: str, branch_name_pattern: str) -> bool:
    ...
    status, headers, data = repo._requester.requestJson(
        "DELETE", f"{repo.url}/environments/{environment_name}/deployment-branch-policies/{branch_policy_id}"
    )
    if status not in {204}:
        raise Exception(f"Unable to delete deployment branch policy for environment {environment_name} with branch policy id {branch_policy_id}. Status: {status}. Error: {json.loads(data)['message']}")
...
def create_environment(repo: Repository, env: environment) -> bool:
    ...
    if status not in {200}:
        raise Exception(f"Unable to create environment: {env.name}. Status: {status}. Error: {json.loads(data)['message']}")
...
def check_environment_settings(repo: Repository, config_env: environment) -> tuple[bool, dict[str, Any]]:
    ...
    repo_protection_rules_dict = {protection_rule.type: protection_rule for protection_rule in repo_env.protection_rules}
    diffs = {
        "wait_timer": None,
        "branch_policy": {},
        "required_reviewers": {},
    }
    keys = {"wait_timer", "branch_policy", "required_reviewers"} # can't use keys() because it's changes during iterator
    for protection_rule in keys:
        if protection_rule == "required_reviewers":
            ...
            if repo_reviewers is None:
                ...
                repo_reviewers = {reviewer.reviewer.login: Reviewer(type=reviewer.type, name=reviewer.reviewer.login) for reviewer in repo_reviewers.reviewers}
...
def check_repo_environments(repo: Repository, environments: list[environment]) -> tuple[bool, dict[str, list[str] | dict[str, Any]]]:
                    ...
                    if env_component == "branch_policies":
                        ...
                        for branch_policy_issue_type in branch_policy_issue_types:
                            if branch_policy_issue_type == "missing":
                                for branch_name_pattern in diffs[issue_type][env_name][env_component][branch_policy_issue_type]:
                                    pErrors.append(f"Unable to link deployment environment {env_name} to branches {branch_name_pattern}. Currently the GitHub API does not support creating branch policies for environments; get error 404...")
                        ...
                        if issue_type == "missing":
                            diffs[issue_type][env_name][env_component] = {"missing": [variable.key for variable in config_env_dict[env_name].variables]}
                        pErrors = update_variables(repo, config_env_dict[env_name].variables, diffs[issue_type][env_name][env_component])
                    if len(pErrors) > 0:
